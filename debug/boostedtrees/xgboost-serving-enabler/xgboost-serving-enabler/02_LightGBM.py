...
# MAGIC
# MAGIC To overcome this limitation, many popular models, such as those in the LightGBM family of models, have implemented capabilities allowing them to process data in Spark dataframes.  Spark dataframes overcome the memory limitations of a single server by allowing large datasets to be distributed over the combined resources of the multiple servers that comprise a Spark cluster. When models implement support for Spark dataframes, all or portions of the work they perform against the data can be distributed in a similar manner.
# MAGIC
...
# DBTITLE 1,Verify the Data File in Mount Point
display(
...
# MAGIC %md Next, we need to ensure our numerical columns do not contain any missing values.  If you are familiar with other implementations of the LightGBM models, you may be curious why we need to impute missing values here.  The reason is not because of the Spark-distributed LightGBM model but instead the VectorAssembler we will use in a bit.  The VectorAssembler requires that all incoming features have values assigned to them:
...
# MAGIC
# MAGIC If you are familiar with the use of LightGBM models, you may be curious why it is we need to assemble a vector at this point. With traditional LightGBM models, data are often passed in directly from pandas dataframes (or numpy arrays). While the use of a pandas dataframe gives the sense that LightGBM is directly engaging specific fields, it is in fact viewing each record as an array.  In Spark MLlib, an array and a vector are very similar constructs.  The explicit use of a vector with Spark-distributed LightGBM models makes clear how data is being handed to the model and aligns the approach with what is typically used with Spark MLlib:
...
# MAGIC %md We can then configure the [Spark-distributed Synapse LightGBM Regressor](https://mmlspark.blob.core.windows.net/docs/0.11.1/pyspark/synapse.ml.lightgbm.html#module-synapse.ml.lightgbm.LightGBMRegressor) as follows:
...
pyspark_pipeline = Pipeline(stages=[string_indexer, ohe_encoder, imputer, vec_assembler, lgb_regressor])

# COMMAND ----------

# MAGIC %md With the model pipeline assembled, we can now train it across the Databricks cluster. Please note that we are not performing the hyperparameter tuning that's typically done with LightGBM models as our focus is on deployment patterns.  If you'd like to see an example of how hyperparameter tuning can be performed with Spark-distributed LightGBM models, please check out the sample notebook provided [here](https://docs.databricks.com/machine-learning/train-model/xgboost-spark.html) and adapt the logic for the LightGBM model:
...
# DBTITLE 1,Extract Booster Data from Distributed Model
# retrieve the trained model
pyspark_model = pyspark_trained.stages[-1]

# save the native booster model to disk
pyspark_model.saveNativeModel('dbfs:/tmp/lgbbooster')

# retrieve the booster data in the one text file in the saved model folder
saved_model = pathlib.Path('/dbfs/tmp/lgbbooster')
for i in saved_model.iterdir():
  if str(i)[-4:] == '.txt':
    with open(i) as text_file:
      booster_data = text_file.read()
    break

# display this booster data
print(booster_data)

# COMMAND ----------

# MAGIC %md We can then use this data to instantiate our model.  One challenge with the LightGBM model is that it does not have a *fit* method which is required to incorporate it into our sklearn pipeline.  So, we'll create a class wrapper with a dummy *fit* method to avoid a problem with deployment:

# COMMAND ----------

# DBTITLE 1,Instantiate Sklearn Model Using Booster Data
# define dummy class for model 
class alt_lgb(lgb.Booster):
  def fit(self):
    print('x')

# instantiate model
sklearn_model = alt_lgb(model_str=booster_data)
...
# MAGIC %md Next we need to ensure the host that runs our model is aware of our dependency on LightGBM:
...
model_version = client.search_model_versions(f"name='{model_name_lgb}'")[0].version
...
# retrieve model
loaded_model = mlflow.pyfunc.load_model(f"models:/{model_name_lgb}/production")
...
# MAGIC
# MAGIC Now that we are comfortable our logged model is working properly, we can deploy it to model serving. To deploy our model, we need to reconfigure our Databricks workspace for Machine Learning.  We can do this by clicking on the drop-down at the top of the left-hand navigation bar and selecting *Machine Learning*.
# MAGIC </p>
# MAGIC
...
# MAGIC |----------------------------------------|-------------------------|------------|-----------------------------------------------------|
# MAGIC | SynapseML  | Open-source library that simplifies the creation of massively scalable machine learning (ML) pipelines |  MIT | https://github.com/Microsoft/SynapseML  |